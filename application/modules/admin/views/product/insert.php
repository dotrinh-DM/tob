...
if(isset($edit_record) && count($edit_record) > 0) {
    ...
    $cate           = $edit_record['id_category'];

    if($edit_record['image'] != null) {
        $avatar = base_url().'uploads/product/'.$edit_record['image'];
    }
...
<h4 class="widgettitle nomargin shadowed"><?php echo $this->lang->line('pro_form'); ?></h4>
<div class="widgetcontent bordered shadowed nopadding">
    <form class="stdform stdform2" id="frmuser" method="post" action="" enctype="multipart/form-data">

        <p>
            <label><?php echo $this->lang->line('product'); ?> (*) <?php echo "<small style='color:tomato'>".form_error('name')."</small>"; ?></label>
        ...
        <p>
            <label><?php echo $this->lang->line('category'); ?> (*) <?php echo "<small style='color:tomato'>".form_error('cate')."</small>"; ?></label>
                <span class="field">
                    <select name="cate" id="cate" class="uniformselect">
                        <option value="">Choose one</option>
                        <?php
                        if(isset($list_cate) && count($list_cate) > 0) {
                            foreach($list_cate as $key) {
                            ?>
                                <option value="<?php echo $key['id_category']; ?>" <?php if($cate != '' && ($cate == $key['id_category'])) {echo "selected='selected'";} else {echo set_select('cate', $key['id_category']);} ?> ><?php echo $key['name']; ?></option>
                            <?php
                            }
                        }
                        ?>
        ...
        <p>
            <label><?php echo $this->lang->line('brand'); ?> <?php echo "<small style='color:tomato'>".form_error('brand')."</small>"; ?></label>
                <span class="field">
                    <select name="brand" id="brand" class="uniformselect">
                        <option value="">Choose one</option>
                        <?php
                        if(isset($list_brand) && count($list_brand) > 0) {
                            foreach($list_brand as $key) {
                                ?>
                                <option value="<?php echo $key['id_brand']; ?>" <?php if($cate != '' && ($cate == $key['id_brand'])) {echo "selected='selected'";} else {echo set_select('cate', $key['id_brand']);} ?> ><?php echo $key['name']; ?></option>
                            <?php
                            }
                        }
                        ?>
        ...
        <p>
            <label><?php echo $this->lang->line('image_pro'); ?>
                <?php
        ...
        <p>
            <label><?php echo $this->lang->line('desc_pro'); ?></label>
